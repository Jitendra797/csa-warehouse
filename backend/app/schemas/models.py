...
class RunPipelineResponse(BaseModel):
    status: Literal["running", "success", "failed"] = Field(..., description="The current status of the pipeline")
...
class Role(BaseModel):
    ...
    created_at: datetime = Field(default_factory=datetime.now, description="Timestamp when the role was created")
    updated_at: datetime = Field(default_factory=datetime.now, description="Timestamp when the role was last updated")
...
class User(BaseModel):
    ...
    external_id: str = Field(..., description="External ID from OAuth provider (e.g., Google sub)")
    role_id: Optional[str] = Field(None, description="Role ID from roles collection")
    created_at: datetime = Field(default_factory=datetime.now, description="Timestamp when the user was created")
    updated_at: datetime = Field(default_factory=datetime.now, description="Timestamp when the user was last updated")
...
class UserResponse(BaseModel):
    ...
    email: Optional[str] = Field(None, description="Email address")
    phone: Optional[str] = Field(None, description="Phone number")
    external_id: str = Field(..., description="External ID from OAuth provider")
    role_id: Optional[str] = Field(None, description="Role ID assigned to the user")
...
class EndpointAccess(BaseModel):
    ...
    contributor: bool = Field(default=False, description="Contributor permission")
