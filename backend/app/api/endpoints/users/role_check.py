...
def extract_user_id_from_token(authorization: str) -> str:
    ...
    if not authorization or not authorization.startswith("Bearer "):
        logger.warning(
            "Missing or invalid Authorization header while extracting user id")
        raise HTTPException(
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.error("Google UserInfo request failed: %s %s",
...
def find_matching_endpoint_access(role: str, path: str):
    ...
    sorted_access = sorted(all_access, key=lambda x: len(
        x.get("endpoint", "")), reverse=True)
...
def check_user_role_access(request: RoleCheckRequest, fastapi_request: Request, authorization: str = Header(None)):
    ...
    """
    try:
        logger.info("Role check requested: path=%s",
        ...
        logger.debug("User role resolved: role_id=%s role_name=%s",
                     role_id, role_name)

        # Find matching endpoint access rule
        endpoint_access = find_matching_endpoint_access(
            role_name, request.path)
        logger.debug("Matched endpoint access: %s", endpoint_access)

        if not endpoint_access:
            # No access rule found, deny access
            logger.info(
                "No endpoint access rule found: role=%s path=%s", role_name, request.path)
        ...
        logger.warning("HTTPException during role check: status=%s detail=%s",
                       http_exc.status_code, http_exc.detail)
        raise
    except Exception as e:
        logger.exception("Unhandled error during role check for path=%s", getattr(
            request, 'path', None))
        raise HTTPException(
            status_code=500, detail=f"Error checking role access: {str(e)}")
...
def initialize_endpoint_access():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error initializing endpoint access: {str(e)}")
